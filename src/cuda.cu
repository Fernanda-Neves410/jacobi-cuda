...
__global__ void kernel_calcula_erro(const double *x_old, const double *x_new, double *erro_global_sq, int n) {
...
void executar_jacobi_cuda(double *h_A, double *h_b, double *h_x_final, int n, double epsilon, int max_iter) {
    ...
    while (iter < max_iter && erro_atual > epsilon) {
        ...
        kernel_calcula_erro<<<blocos_na_grade, threads_por_bloco, shared_mem_size>>>(d_x_old, d_x_new, d_erro, n);
